...
if(RADAR_RANGING):
    #number of samples to plot in the amplitude/time plot
    TIME_NUM_SAMPLES = int(round(20/1000.0 * SAMPLING_RATE))
    FFT_N = 2048 * 4
            ...
            while(True): #this loop processes frames
                ...
                if(bufferIdx is not None):
                    ...
                    if(negEdges.size > 0):
                        ...
                        a = buffer[bufferIdx:]
                        b = time_[:negEdges[0]]
                        if (a.size + b.size > MINIMUM_FRAME_LENGTH):
                    ...
                    if(posEdges.size > 0):
                        ...
                        if(negEdges.size>0):
                            ...
                            if (frame.size > MINIMUM_FRAME_LENGTH):
                                print 'found frame. size: ' + str(frame.size)
                                self.process_frame(frame)
